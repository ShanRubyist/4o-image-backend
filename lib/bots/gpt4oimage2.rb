...
module Bot
  class Gpt4oimage2 < AIModel
    def initialize(api_key = ENV.fetch('KIE_API_KEY'), api_base_url = 'https://kieai.erweima.ai')
      @api_key = api_key
      @api_base_url = api_base_url
    end

    def image_api(content, options = {})
      path = options.fetch(:path, '/api/v1/gpt4o-image/generate')
